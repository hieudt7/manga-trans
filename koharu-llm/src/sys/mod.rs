...
fn load_libraries(dir: &Path) -> Result<LoadedLibraries> {
    ...
    let ggml_base = load_and_bind(dir, ggml_base_name, |lib| unsafe {
        generated::ggml_base::ggml_base::from_library(lib)
    })?;
    let ggml = load_and_bind(dir, ggml_name, |lib| unsafe {
        generated::ggml::ggml::from_library(lib)
    })?;
    let llama = load_and_bind(dir, llama_name, |lib| unsafe {
        generated::llama::llama::from_library(lib)
    })?;
    let mtmd = load_and_bind(dir, mtmd_name, |lib| unsafe {
...
fn load_and_bind<T>(
    ...
    let path_str = full_path
        .to_str()
        .with_context(|| format!("path for `{name}` is not valid UTF-8"))?;
    let library = koharu_runtime::load_library_by_name(path_str)
