...
pub fn router(resources: SharedResources, events: EventHub) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/documents/{document_id}/text-blocks/bulk",
            put(import_text_blocks_json),
        )
        .route(
...
async fn save_rendered_document(
    State(state): State<ApiState>,
    Path(document_id): Path<String>,
) -> ApiResult<StatusCode> {
    let resources = state.resources()?;
    let (index, _) = find_document(&resources, &document_id).await?;
    operations::save_rendered(resources, IndexPayload { index })
        .await
...
async fn import_text_blocks_json(
    State(state): State<ApiState>,
    Path(document_id): Path<String>,
    Json(text_blocks): Json<Vec<TextBlockDetail>>,
) -> ApiResult<StatusCode> {
    let resources = state.resources()?;
    let (index, _) = find_document(&resources, &document_id).await?;
        ...
        |document| {
            document.text_blocks = text_blocks
                ...
                .map(|detail| TextBlock {
                    ...
                    line_polygons: detail.line_polygons.clone(),
                    source_direction: detail.source_direction,
                    rendered_direction: detail.rendered_direction,
                    source_language: detail.source_language.clone(),
                    rotation_deg: detail.rotation_deg,
                    detected_font_size_px: detail.detected_font_size_px,
                    detector: detail.detector.clone(),
                    text: detail.text.clone(),
                    translation: detail.translation.clone(),
                    style: detail.style.clone(),
                    font_prediction: detail.font_prediction.clone(),
                    ..Default::default()
                })
                .collect();
            Ok(())
