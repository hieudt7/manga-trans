...
export const useDocumentMutations = () => {
  ...
  const saveRendered = useCallback(
    async (_: null | undefined, index?: number) => {
      const resolvedIndex =
        index ?? useEditorUiStore.getState().currentDocumentIndex
      await api.saveRendered(resolvedIndex)
    },
    [],
  )
  ...
  const deleteDocument = useCallback(async () => {
    ...
    await api.deleteDocument(currentDocumentIndex)
    const newTotal = totalPages - 1
    setTotalPages(newTotal)
    const nextIndex = Math.min(currentDocumentIndex, newTotal - 1)
    setCurrentDocumentIndex(Math.max(0, nextIndex))
    queryClient.setQueryData(queryKeys.documents.count, newTotal)
    await refreshDocuments()
  }, [queryClient, refreshDocuments])

  const exportDocumentJson = useCallback(async () => {
    const { currentDocumentIndex } = useEditorUiStore.getState()
    await api.exportDocumentJson(currentDocumentIndex)
  }, [])

  const importDocumentJson = useCallback(async () => {
    const { currentDocumentIndex } = useEditorUiStore.getState()
    await api.importDocumentJson(currentDocumentIndex)
...
export const useLlmMutations = () => {
  ...
  const llmToggleLoadUnload = useCallback(async () => {
    ...
    const apiKey =
      modelInfo && modelInfo.source !== 'local'
        ? modelInfo.source === 'openai-compatible'
          ? usePreferencesStore.getState().localLlm.apiKey?.trim() ||
            usePreferencesStore.getState().apiKeys[modelInfo.source]
          : usePreferencesStore.getState().apiKeys[modelInfo.source]
        : undefined
    const baseUrl =
    ...
    const { providerStoryContexts, providerCustomPrompts } =
      usePreferencesStore.getState()
    const isLocalCompatible = modelInfo?.source === 'openai-compatible'
    const providerSource = modelInfo?.source
    const storyContext = isLocalCompatible
      ? localLlmConfig.storyContext || undefined
      : providerSource
        ? providerStoryContexts[providerSource] || undefined
        : undefined
    const customSystemPrompt = isLocalCompatible
      ? localLlmConfig.customSystemPrompt || undefined
      : providerSource
