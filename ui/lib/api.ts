...
export const api = {
  ...
  async exportDocumentJson(index: number): Promise<void> {
    return withRpcError('export_document_json', async () => {
      const summary = await getDocumentSummaryAtIndex(index)
      const detail = await fetchJson<DocumentDetail>(`/documents/${summary.id}`)
      const blob = new Blob([JSON.stringify(detail, null, 2)], {
        type: 'application/json',
      })
      try {
        await fileSave(blob, {
          fileName: `${summary.name}_koharu.json`,
        })
      } catch {}
    })
  },

  async importDocumentJson(index: number): Promise<void> {
    return withRpcError('import_document_json', async () => {
      const file = await fileOpen({
        mimeTypes: ['application/json'],
        extensions: ['.json'],
      })
      const text = await file.text()
      const detail = JSON.parse(text) as DocumentDetail
      const summary = await getDocumentSummaryAtIndex(index)
      await fetchJson(`/documents/${summary.id}/text-blocks/bulk`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(detail.textBlocks),
      })
      documentDetailCache.delete(summary.id)
    })
  },

  async saveRendered(index: number): Promise<void> {
    return withRpcError('save_rendered', async () => {
      const summary = await getDocumentSummaryAtIndex(index)
      await fetchJson(`/documents/${summary.id}/save-rendered`, {
        method: 'POST',
      })
    })
  },
  ...
  async deleteDocument(index: number): Promise<void> {
    return withRpcError('delete_document', async () => {
      const summary = await getDocumentSummaryAtIndex(index)
      await fetchJson(`/documents/${summary.id}`, { method: 'DELETE' })
      documentDetailCache.delete(summary.id)
    })
  },
