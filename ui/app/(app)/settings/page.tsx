...
export default function SettingsPage() {
  ...
  const providerStoryContexts = usePreferencesStore(
    (state) => state.providerStoryContexts,
  )
  const setProviderStoryContext = usePreferencesStore(
    (state) => state.setProviderStoryContext,
  )
  const providerCustomPrompts = usePreferencesStore(
    (state) => state.providerCustomPrompts,
  )
  const setProviderCustomPrompt = usePreferencesStore(
    (state) => state.setProviderCustomPrompt,
  )
                  ...
                  ({
                    ...
                    <div key={id} className='space-y-1'>
                      ...
                      <div className='space-y-1'>
                        ...
                        {/* Story context */}
                        <textarea
                          value={providerStoryContexts[id] ?? ''}
                          onChange={(e) =>
                            setProviderStoryContext(id, e.target.value)
                          }
                          placeholder={t(
                            'settings.localLlmStoryContextPlaceholder',
                          )}
                          rows={3}
                          className={`${inputClass} resize-y`}
                        />
                        <span className='text-muted-foreground text-xs'>
                          {t('settings.localLlmStoryContextHint')}
                        </span>

                        {/* Custom system prompt */}
                        <textarea
                          value={providerCustomPrompts[id] ?? ''}
                          onChange={(e) =>
                            setProviderCustomPrompt(id, e.target.value)
                          }
                          placeholder={t(
                            'settings.localLlmSystemPromptPlaceholder',
                          )}
                          rows={3}
                          className={`${inputClass} resize-y`}
                        />
              ...
              <div className='space-y-3'>
                ...
                {showAdvanced && (
                  <div className='space-y-3 pl-1'>
                    ...
                    <div className='space-y-1'>
                      <label className='text-foreground text-sm'>
                        {t('settings.localLlmStoryContext')}
                      </label>
                      <textarea
                        value={localLlm.storyContext}
                        onChange={(e) =>
                          setLocalLlm({ storyContext: e.target.value })
                        }
                        placeholder={t(
                          'settings.localLlmStoryContextPlaceholder',
                        )}
