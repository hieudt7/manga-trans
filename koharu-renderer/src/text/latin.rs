...
pub fn balloon_bounds_from_image(block: &TextBlock, image: &GrayImage) -> Option<LayoutBox> {
    ...
    let cx = ((block.x + block.width / 2.0).round() as i32).clamp(0, img_w - 1);
    let cy = ((block.y + block.height / 2.0).round() as i32).clamp(0, img_h - 1);

    if image.get_pixel(cx as u32, cy as u32)[0] < INTERIOR_THRESHOLD {
        return None;
    }

    // Scan at three y positions to handle non-rectangular balloons.
    let dy = (block.height as i32 / 4).max(0);
    let scan_ys = [cy - dy, cy, cy + dy];
    let mut left = cx;
    let mut right = cx + 1;
    for y in scan_ys {
        ...
        let row_left = (0..cx)
            .rev()
            .find(|&x| image.get_pixel(x as u32, y as u32)[0] < INTERIOR_THRESHOLD)
            .map(|x| x + 1)
            .unwrap_or(0);
        let row_right = (cx..img_w)
            .find(|&x| image.get_pixel(x as u32, y as u32)[0] < INTERIOR_THRESHOLD)
            .unwrap_or(img_w);
    ...
    // Scan at three x positions for vertical extent.
    let dx = (block.width as i32 / 4).max(0);
    let scan_xs = [cx - dx, cx, cx + dx];
    let mut top = cy;
    let mut bottom = cy + 1;
    for x in scan_xs {
        ...
        let col_top = (0..cy)
            .rev()
            .find(|&y| image.get_pixel(x as u32, y as u32)[0] < INTERIOR_THRESHOLD)
            .map(|y| y + 1)
            .unwrap_or(0);
        let col_bottom = (cy..img_h)
            .find(|&y| image.get_pixel(x as u32, y as u32)[0] < INTERIOR_THRESHOLD)
            .unwrap_or(img_h);
        top = top.min(col_top);
        bottom = bottom.max(col_bottom);
    }

    let x0 = (left + TEXT_PADDING).min(right.saturating_sub(1));
    let y0 = (top + TEXT_PADDING).min(bottom.saturating_sub(1));
    let x1 = (right - TEXT_PADDING).max(x0 + 1);
    let y1 = (bottom - TEXT_PADDING).max(y0 + 1);
    let w = (x1 - x0) as f32;
    let h = (y1 - y0) as f32;
